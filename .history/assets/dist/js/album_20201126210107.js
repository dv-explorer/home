...
function setupInteraction() {
    ...
    // activate scroll spy
    $("#card-display").scroll(displaySpy);
    ...
    $.getJSON(NS_doc, function (json) {
        $.each(json, function (i, item){
            ...
            currentDisplayPart
                ...
                .append($("<div></div>").addClass("row row-cols-1 row-cols-sm-2 row-cols-lg-3 card-deck"));  // create card deck
...
function cardOver() {
    var target = $(this).parentsUntil(".card-inner").last();
    // turn over to back
...
function NS_scroller() {
    ...
    var target = document.querySelector(targetId).offsetTop;
    // console.log("target ID: " + targetId + "\ntarget Y: " + target);
    $(this).parent().find(".active").removeClass("active");
    $(this).addClass("active");
    $('#card-display').animate({scrollTop: target}, 500, "easeInOutQuart");
}
...
function displaySpy() {
    ...
    if(document.querySelector(".deck-reminder.active-sticky")) {
        ...
        var dist = currentReminder.getBoundingClientRect().top - currentDeck.getBoundingClientRect().bottom;
        if(dist < 0.5 * document.querySelector("#card-display").offsetHeight) {
            if($(currentReminder).css("display") != "none") {
                $(currentReminder).css("display") = "none";
            }
        } else if($(currentReminder).css("display") == "none") {
            currentReminder.style.visibility = "block";
        }
    }
...
function searchFunc() {
    ...
    if(readReg.length > 0 && (readReg[0] != ("" | " "))) {
        ...
        if(readReg.length === 1 && $($("#" + read.toLowerCase()).parent()[0]).attr("id") === "card-display") {
            ...
            console.log("Search finished 01");
            ...
            $.getJSON(card_doc, function(json) {
                ...
                $.each(json, function(i, item) {
                    ...
                    if(i == 42) {console.log("Search finished 02");}
                });

            }).done(function() {
                console.log("ready to manipulate deckDisplay.");
...
function deckDisplay(list, idString) {
    ...
    document.querySelectorAll("#card-display > div").forEach((part, i, parent) => {
        if($(part).find(".trans-3d:visible").length == 0) {
            ...
            console.log("456_" + $(part).find(".trans-3d:visible").length);
            $("." + $(part).attr("id")).removeClass("disabled");
        }
    });
