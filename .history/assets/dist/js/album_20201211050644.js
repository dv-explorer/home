...
function createNS(NS_doc, classStr) {
    ...
    $.getJSON(NS_doc, function (json) {
        $.each(json, function (i, item){
            ...
            var display_title = $("<h2></h2>").addClass("display-title").text(NS_tag.replace(NS_tag[0], NS_tag[0].toUpperCase()) + " (" + NS_num + ")");
...
function createDisplay(cards_doc) {
    ...
    $.getJSON(cards_doc, function(json) {
        $.each(json, function(id, card) {
            ...
            let front_info = {"how": card.how, "why": card.why, "avt": card.avt};
            let back_caption = {
                "title":card.eg_title,
