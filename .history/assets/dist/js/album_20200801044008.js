...
function setupInteraction() {
    ...
    function ImgLoading(status) {
        ...
        img.onload = function() {
            // callback.call(img);
            $($(parent).children("img")[0]).replaceWith(this);
            $(this).addClass(cls);
            if(status == "front") {
                const logo = new Freezeframe($(this));
                return;
    ...
    $(".front").each(function(){ImgLoading("front");});
    $(".back").each(function(){ImgLoading("back");});
...
function drawCardFront(card_id, card_color, ds_tag, EL_tag, url, front_info) {
    ...
    var frontGif = $("<img />").addClass("card-img")
        .attr({
            ...
            "data-target": "assets/front_gif/front_" + card_id + ".gif"
        });
...
function drawCardBack(card_id, card_color, ds_tag, EL_tag, url, back_caption) {
    ...
    var back_gif = $("<img />").addClass("card-img")
        .attr({
            src: "assets/media/loading.svg",
            "data-target": "assets/back_gif_compressed/back_" + card_id + ".gif"
        });
