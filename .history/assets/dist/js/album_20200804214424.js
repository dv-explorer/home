...
function displaySpy() {
    ...
    var currentPosition = $(item).position().top - $(window).scrollTop();
        if((currentPosition < 0.5*screenH) && (($(item).height() + currentPosition) >= 0.5*screenH)) {
            ...
            $('.btn-primary-group-sm').animate({scrollLeft: $(".btn-primary-group-sm > .active").offset().left + $('.btn-primary-group-sm').css("padding-left")});
            console.log($(".btn-primary-group-sm > .active").offset().left);
        }
        // ".btn-primary-group-sm > .active"
    });
}
