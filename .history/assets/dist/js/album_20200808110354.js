...
function searchFunc() {
    ...
    if(read.length > 0 && (read[0] != ("" | " "))) {
        ...
        $.getJSON(card_doc, function(json) {
            ...
            console.log(show_list, " is ready for hide/show.");
...
function deckDisplay(list) {
    ...
    $(".trans-3d").hide("fast");
    console.log($(".trans-3d:visible").length, " can be seen.");
    ...
    // function hideOrNot(){
        $.ajaxSettings.async = false;
        $("#card-display > div").each(function(i, part) {
        // $.map($("#card-display > div"), function(part){
            // console.log($(part).attr("id"));
            console.log($(part).attr("id"), $(part).find(".trans-3d:visible").length, " debugging...");
            if($(part).find(".trans-3d:visible").length == 0) {
                $(part).slideUp("fast");
                console.log($(part).attr("id"), " is sliding up.");
                $("." + $(part).attr("id") + ":not(disabled)").addClass("disabled");
            } else {
                console.log($(part).attr("id")," no move");
                $("." + $(part).attr("id")).removeClass("disabled");
            }
        });
    // }
